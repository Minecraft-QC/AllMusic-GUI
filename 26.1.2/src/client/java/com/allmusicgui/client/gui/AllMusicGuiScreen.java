...
public class AllMusicGuiScreen extends Screen {
	...
	private void buildLocalPage(int lx) {
		if (namingPlaylist) return; // 弹窗由 buildPopup 处理
		if (currentPlaylistId == null) {
			// 一级：歌单列表
	...
	public boolean keyPressed(KeyEvent e) {
		...
		if (e.key() == GLFW_KEY_ESCAPE) {
			...
			return super.keyPressed(e); // 一级：默认关闭界面
		}
		...
		if ((e.key() == GLFW_KEY_ENTER || e.key() == GLFW_KEY_KP_ENTER) && nameBox != null && nameBox.isFocused()) {
			confirmCreate();
			return true;
		}
		...
		if (!inputFocused && !pickingPlaylist && !namingPlaylist && list != null && list.hasSongs()) {
			...
			if ((e.key() == GLFW_KEY_DELETE || e.key() == GLFW_KEY_BACKSPACE) && list.hasSelection()) {
				// 本地歌单二级：移除；其他页：删除选中本地歌单内歌曲
				if (tab == Tab.LOCAL && currentPlaylistId != null) {
	...
	private void drawContent(GuiGraphicsExtractor g, int mx, int my) {
		...
		if (tab == Tab.MINE) {
			...
			if (tab == Tab.LOCAL && currentPlaylistId == null && store.localPlaylists().isEmpty()) {
				g.centeredText(font, "还没有本地歌单，点「新建歌单」创建", width / 2, height / 2 - 4, 0xFF8F8F8F);
			}
		}
	...
	private void drawPopup(GuiGraphicsExtractor g, int mx, int my) {
		...
		g.centeredText(font, namingPlaylist ? "新建歌单" : "选择歌单（收藏到）", cx, py + 8, 0xFFFFFFFF);
		if (namingPlaylist && pendingSong != null) {
			g.text(font, "将收藏：" + pendingSong.name, px + 30, py + 24, 0xFFC0C0C0);
		}
		if (pickingPlaylist && pendingSong != null) {
			g.text(font, pendingSong.name, px + 30, py + 20, 0xFFC0C0C0);
		}
		g.text(font, "ESC 取消", px + POPUP_W - 60, py + ph - 14, 0xFF6E6E6E);
	}
	...
	private void renderMine(GuiGraphicsExtractor g, int lx, int mx, int my) {
		...
		g.text(font, "本地歌单 · 喜欢（" + likeCount + "）", lx + 4, mineTop() + 52, 0xFFA0A0A0);
