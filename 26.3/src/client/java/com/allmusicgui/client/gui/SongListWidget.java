...
/** 简易可滚动歌单列表（26.x 渲染：GuiGraphicsExtractor；原版风格，悬停/选中/点击回调）。
 *  注意：26.x 的 text() 要求颜色带非零 alpha，否则整段被丢弃。 */
public class SongListWidget {
	...
	public void render(GuiGraphicsExtractor g, int mx, int my) {
		...
		for (int i = scroll; i < Math.min(songs.size(), scroll + visible + 1); i++) {
			...
			int bg = hover ? 0x40FFFFFF : (i == selected ? 0x60FFFFFF : 0x00000000);
			if (bg != 0) g.fill(x, ry, x + w, ry + ROW_H, bg);
			g.text(font, (i + 1) + ". " + s.name, x + 4, ry + 3, 0xFFFFFFFF);
			g.text(font, s.sub, x + 4, ry + 14, 0xFF9B9B9B);
			String right = s.durationText().isEmpty() ? s.type : s.durationText() + "  " + s.type;
			g.text(font, right, x + w - font.width(right) - 4, ry + 7, 0xFF8A8A8A);
		}
		g.disableScissor();
	}
	...
	/** @param amount 垂直滚动量；>0 向上滚（看更早的条目） */
